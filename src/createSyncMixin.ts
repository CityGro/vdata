...
const toKV: (o: any) => array = flow(entries, flatten)
...
export default (valueProp) => {
    ...
    return {
        methods: {
            [format('forwardInput', prefix)] (e): void {
                this.$emit(event, e)
            },
            [format('handleChange', prefix)] (value): void {
            ...
            [format('handleKeyChange', prefix)] (key, value): void {
                this.handleChange({[key]: {...this[valueProp][key], ...value}})
            },
            [format('handleArrayKeyChange', prefix)] (i, key, value): void {
                let arr = [...this[valueProp][key]]
                arr[i] = {...arr[i], ...value}
                this.handleChange({[key]: arr})
            },
            [format('handleArrayChange', prefix)] (i, value): void {
                let arr = [...this[valueProp]]
                arr[i] = {...arr[i], ...value}
                this.$emit(event, arr)
            },
            [format('pushToArray', prefix)] (value): void {
                let arr = [...this[valueProp]]
                arr.push(value)
                this.$emit(event, arr)
            },
            [format('pushToArrayKey', prefix)] (key, value): void {
                let arr = [...this[valueProp][key]]
                arr.push(value)
                this.handleChange({[key]: arr})
            },
            [format('removeFromArray', prefix)] (i): void {
                let value = [...this[valueProp]]
                value.splice(i, 1)
                this.$emit(event, value)
            },
            [format('removeFromArrayKey', prefix)] (i, key): void {
                let arr = [...this[valueProp][key]]
                arr.splice(i, 1)
                this.handleChange({[key]: arr})
            }
